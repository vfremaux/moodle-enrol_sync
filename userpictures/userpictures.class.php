    ...
    function cron() {
		...
		foreach($filestoprocess as $f){
            ...
            if (!$unzipresult) {
                ...
                remove_dir($zipdir);
                enrol_sync_report($CFG->userpictureslog, get_string('usersupdated', 'enrol_sync') . ": " . $results['updated']);
                enrol_sync_report($CFG->userpictureslog, get_string('errors', 'enrol_sync') . ": " . $results['errors']);
                echo '<hr />';
            }
