...
// security
    ...
    $url = $CFG->wwwroot.'/enrol/sync/courses/deletecourses.php';
	...
	echo $OUTPUT->header();
	echo $OUTPUT->heading_with_help(get_string('coursedeletion', 'enrol_sync'), 'coursedeletion', 'enrol_sync');
	...
	if(!isset($_POST['ids'])) {
	    ...
	    if ($um->preprocess_files() || isset($_POST['uselocal'])) {
		    ...
		    if (isset($um->files['deletefileupload'])) {
				// All file processing stuff will go here. ID=2...
		  		echo $OUTPUT->notification(get_string('parsingfile', 'enrol_sync'), 'notifysuccess');		
		...
		if (isset($filename)){
			...
			foreach($identifiers as $cid) {
				if(!($c = $DB->get_record('course', array($identifiername => $cid))) ) {
					...
					echo $OUTPUT->notification(get_string('coursenodeleteadvice', 'enrol_sync', $cid), 'notifyproblem');
					continue;
				}
	        ...
	        echo $OUTPUT->notification(get_string('totaltime', 'enrol_sync').' '.round(($time_end - $time_start),2).' s', 'notifysuccess');
			...
			echo $OUTPUT->notification(get_string('nofile', 'enrol_sync'), 'notifyproblem');
		}
