			...
			if(isset($record['cmd'])){				
				if($record['cmd'] == 'del'){
					...
					if ($role = get_record('role', 'shortname', $record['rolename'])){
						...
						if ($var == '1'){
							//TODO ajout du role en hidden
							if(!get_record('role_assignments', 'roleid', $role->id, 'contextid', $context->id, 'userid', $user->id)){
								if (!role_assign($role->id, $user->id, null, $context->id,0,0,1,'flatfile')){
									if (!empty($CFG->sync_filefailed)) sync_feed_tryback_file($filename, $text, $headers);
				...
				if (empty($CFG->enrol_defaultcmd)) {
					...
					if($cmd == 'del'){
						...
						if($role = get_record('role', 'shortname', $record['rolename'])){
							...
							// Role added in the required course
						} else {
							if (!empty($CFG->sync_filefailed)) sync_feed_tryback_file($filename, $text, $headers);
