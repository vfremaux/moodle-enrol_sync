...
*/
	...
	$url = $CFG->wwwroot.'/enrol/sync/courses/resetcourses.php';
	$PAGE->set_url($url);
	$PAGE->set_context(null);
	$PAGE->navigation->add(get_string('synchronization', 'enrol_sync'), $CFG->wwwroot.'/enrol/sync/sync.php');
	$PAGE->navigation->add(get_string('coursereset', 'enrol_sync'), null);
	$PAGE->set_title("$site->shortname");
	$PAGE->set_heading($site->fullname);
	echo $OUTPUT->header();

	echo $OUTPUT->heading(get_string('resettingcourses', 'enrol_sync'));
        ...
        if (file_exists($filename)) {
			...
			while (!feof ($fp)) {
				...
				if ($record['events'] == 'yes') {		
					...
					enrol_sync_report($CFG->resetlog, get_string('noeventstoprocess', 'enrol_sync', $i), false);
				}
				...
				if ($record['logs'] == 'yes') {		
					...
					enrol_sync_report($CFG->resetlog, get_string('nologstoprocess', 'enrol_sync', $i), false);
				}
				// processing notes
				if ($record['notes'] == 'yes') {
					$data['reset_notes'] = 1;
				} else {
					enrol_sync_report($CFG->resetlog, get_string('nonotestoprocess', 'enrol_sync', $i), false);
				}			
				...
				if ($record["grades"] == 'items') {		
					...
					enrol_sync_report($CFG->resetlog, get_string('nogradestoprocess', 'enrol_sync', $i), false);
				}							
				...
				if ($record['groupes'] == 'groupes') {		
					...
					enrol_sync_report($CFG->resetlog, get_string('nogrouptoprocess', 'enrol_sync', $i), false);
				}													
				...
				enrol_sync_report($CFG->resetlog, "Summary:", false);
				foreach($status as $line){
					...
					enrol_sync_report($CFG->resetlog, $str, false);
				}
				$data = array();;
