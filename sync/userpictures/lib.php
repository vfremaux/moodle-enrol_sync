 ...
 * @return nothing
 */
function sync_process_directory ($dir, $userfield, $overwrite, &$results) {
    global $OUTPUT, $CFG;
    ...
    while (false !== ($item = readdir($handle))) {
        if ($item != '.' && $item != '..') {
            if (is_dir($dir.'/'.$item)) {
                sync_process_directory($dir.'/'.$item, $userfield, $overwrite, $results);
            } else if (is_file($dir.'/'.$item))  {
                $result = sync_process_file($dir.'/'.$item, $userfield, $overwrite);
                switch ($result) {
                    case PIX_FILE_ERROR:
                        $results['errors']++;
                        break;
 ...
 *                  PIX_FILE_SKIPPED
 */
function sync_process_file ($file, $userfield, $overwrite) {
