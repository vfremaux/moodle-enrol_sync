...
*/
    ...
    function html_header($wdir, $formfield = ''){
        global $CFG, $ME, $choose, $USER, $SITE;

        $navlinks = array();
        $navlinks[] = array('name' => get_string('enrolname', 'enrol_sync'), 'link' => $CFG->wwwroot.'/enrol/sync/sync.php?sesskey='.$USER->sesskey, 'type' => 'url');
        ...
        if ($wdir == '/') {
            ...
            $navlinks[] = array('name' => $strfiles,
                                'link' => $ME."?wdir=/&amp;choose=$choose",
                                'type' => 'misc');

            for ($i = 2 ; $i < $numdirs - 1 ; $i++) {
                $link .= "/".urlencode($dirs[$i]);
                $navlinks[] = array('name' => 'sync',
                                    'link' => $ME."?id=1&amp;wdir=$link&amp;choose=$choose",
                                    'type' => 'misc');
            }
        ...
        if ($choose) {
            ...
            print_header("shortname: $fullname", $SITE->fullname, $navigation, $formfield);
        }
    ...
    if (!empty($dirlist)) {
        ...
        foreach ($dirlist as $dir) {
            ...
            if ($dir == '..') {
                ...
                print_cell('left', '<a href="'.$CFG->wwwroot.'/enrol/sync/file.php?id='.$id.'&amp;wdir='.$fileurl.'&amp;choose='.$choose.'"><img src="'.$CFG->pixpath.'/f/parent.gif" class="icon" alt="" />&nbsp;'.get_string('parentfolder').'</a>', 'name');
                print_cell();
                print_cell();
                print_cell();
        ...
        foreach ($filelist as $file) {
           ...
           $ffurl = get_syncfile_url($fileurl);
            ...
            if ($icon == 'text.gif' || $icon == 'html.gif') {
                ...
                if (!empty($CFG->backup_version) and has_capability('moodle/site:restore', get_context_instance(CONTEXT_SYSTEM))) {
